// Date: 12 Nov 2025
// Controller to handle logic for feedback/report issue tasks
...
exports.addUserFeedback = async (req, res) => {
  try {
    ...
    if (
      !req.body.leader_regd_mobile_no ||
      ...
      return res.status(400).json({
        status: 'error',
        message: 'Alert! Missing required params....'
      });
    }


    // ----------------------------------------------------------
    // User cannot manually provide case number/status
    // ----------------------------------------------------------
    ...
    console.log(
      'addUserFeedback: Attached File:',
      uf_attachment
    );


    // ----------------------------------------------------------
    // Admin cannot create user feedback
    // ----------------------------------------------------------
    ...
    const newEntry = new UserFeedback({
      ...
      // Case number is generated by schema
      uf_case_no: undefined,

      // Attachment comes from uploaded file
      uf_attachment: uf_attachment || null

      // IMPORTANT:
      // uf_status is NOT accepted from frontend.
      // Schema will automatically set it to "Pending".
    });
    ...
    try {
      ...
      if (user?.fcm_token) {
        await notificationService.sendNotification({
          ...
          body: `Your feedback ${newEntry.uf_case_no} has been submitted successfully.`,

          type: 'feedback'
        });
      }
    } catch (notifErr) {
      // Notification failure should not fail feedback creation
    ...
    logger.info(
      `User Feedback added for ${newEntry.leader_regd_mobile_no}, Case: ${newEntry.uf_case_no}`
    );


    return res.status(201).json({
      status: 'success',
      message: 'User Feedback registered successfully',

      data: {
        uf_case_no: newEntry.uf_case_no,
        uf_status: newEntry.uf_status
      }
    });
    ...
    logger.error(
      `addUserFeedback: error: ${err.message}`
    );


    // Cleanup uploaded attachment if database save fails
    if (req.body?.uf_attachment) {
      deleteFile(req.body.uf_attachment);
    }
...
exports.fetchUserFeedbacks = async (req, res, next) => {
  ...
  logger.info(
    `[${requestId}] fetchUserFeedbacks invoked with leader_regd_mobile_no: ${leader_regd_mobile_no}, user_email_id: ${user_email_id}`
  );
  ...
  if (
    ...
    return res.status(400).json({
      error:
        'Missing required parameters: leader_regd_mobile_no and/or user_email_id and/or uf_type'
    });
  }
  ...
  try {
    ...
    const records = await UserFeedback
      .find(finalQuery)
      .sort({ createdAt: -1 });
    ...
    if (!records || records.length === 0) {
      return res.status(404).json({
        error: 'User Feedback Data not found'
      });
    }
    ...
    const updatedRecords = records.map((record) => {
      ...
      if (
        ...
        obj.uf_attachment_url =
          `${hostUrl}/userfeedbacks/${uf_attachment}`;
    ...
    logger.error(
      `[${requestId}] Error fetching User Feedback Data: ${error.message}`,
      {
        stack: error.stack
      }
    );
...
exports.downloadUserFeedbackFile = (req, res) => {
  ...
  console.log(
    'downloadUserFeedbackFile API: Final ImagePath:',
    filePath
  );
  ...
  return res.sendFile(filePath, (err) => {
    ...
    if (err) {
      ...
      console.error(
        'Error sending User feedback attachment file:',
        err.message
      );
      ...
      if (!res.headersSent) {
        return res.status(404).json({
          message:
            'Please check attachment not found or access denied'
        });
      }
    }
  });
...
exports.updateUserFeedback = async (req, res, next) => {
  ...
  try {
    ...
    if (user_type === 'user') {
      ...
      return res.status(403).json({
        error:
          'Oops! You are not authorized to Edit User Feedback Data.'
      });
    }
...
console.log("userfeedback body", req.body);
    if (
      !leader_regd_mobile_no ||
      !uf_case_no
    ) {
    ...
    // ----------------------------------------------------------
    // Validate status if frontend/admin sends it
    // ----------------------------------------------------------
    ...
    if (
      ...
      return res.status(400).json({
        error:
          'Invalid status. Allowed statuses are: Pending, In Progress, Complete, Resolved'
      });
    }


    console.log(
      'updateUserFeedback: Update Filter:',
      {
        leader_regd_mobile_no,
        uf_case_no
      }
    ...
    console.log(
      'updateUserFeedback: Update Payload:',
      updatePayload
    );
    ...
    if (!updated) {
      ...
      return res.status(404).json({
        error:
          'User Feedback Data not found with given identifiers'
      });
    }
    ...
    try {
      ...
      if (notifyUser?.fcm_token) {
        ...
        let body = `Your feedback ${updated.uf_case_no} has been updated.`;

        if (updatePayload.uf_status) {
          title = 'Feedback Status Updated';
          body = `Your feedback ${updated.uf_case_no} status is now ${updated.uf_status}.`;
        }

        if (updatePayload.uf_action_taken_comments) {
          body += ' Admin has added an action/comment.';
        }

        await notificationService.sendNotification({
          token: notifyUser.fcm_token,
          title,
          body,
    ...
    return res.status(200).json({
      ...
      message:
        'User Feedback details updated successfully',
      ...
      data: {
        ...
        uf_action_taken_comments:
          updated.uf_action_taken_comments,
    ...
    logger.error(
      `User Feedback update failed: ${err.message}`
    );
...
exports.findUserFeedback = async (req, res) => {
  ...
  console.log(
    'findUserFeedback: Request Query:',
    req.query
  );
...
// ============================================================
// COUNT ALL USER FEEDBACK / BUG
// ============================================================

exports.countAllUserFeedback = async (
  req,
  res,
  next
) => {
  ...
  try {
    ...
    const count =
      await UserFeedback.countDocuments(
        finalQuery
      );
    ...
    logger.error(
      `countAllUserFeedback API failed: ${err.message}`
    );
...
// ============================================================
// GET ALL USER FEEDBACK BY TYPE + STATUS
// ============================================================

exports.getAllUserFeedbackbyTypenStatus = async (
  req,
  res,
  next
) => {
  ...
  try {
    ...
    if (
      !allowedStatuses.includes(uf_status)
    ) {
    ...
    const updatedRecords =
      records.map((record) => {
        ...
        if (
          obj.uf_attachment &&
          obj.uf_attachment !== 'null'
        ) {

          obj.uf_attachment_url =
            `${hostUrl}/userfeedbacks/${obj.uf_attachment}`;
...
// ============================================================
// COUNT USER FEEDBACK BY TYPE + STATUS
// ============================================================

exports.countAllUserFeedbackbyStatus = async (
  req,
  res,
  next
) => {
  ...
  try {
    ...
    return res.status(200).json({
      ...
      message:
        `UF Type: ${uf_type}, Status: ${uf_status}`,
    ...
    logger.error(
      `Fetching UserFeedback failed: ${err.message}`
    );
...
exports.deleteUserFeedback = async (
  req,
  res,
  next
) => {
  ...
  try {
    ...
    const deleted =
      await UserFeedback.findOneAndDelete({
        leader_regd_mobile_no,
        uf_case_no
      });
    ...
    if (
      deleted.uf_attachment &&
      deleted.uf_attachment !== 'null'
    ) {
    ...
    return res.status(200).json({
      ...
      message:
        'User Feedback Data deleted successfully',
    ...
    logger.error(
      `UserFeedback deletion failed: ${err.message}`
    );
...
exports.deviceInfo = async (
  ...
  try {
    ...
    const useragent =
      `${req.useragent?.platform || 'unknown'} - ${req.useragent?.browser || 'unknown'}`;
...
async function upsertDeviceInfo(
  ...
  if (
    ...
    throw new Error(
      'Missing required identifiers: leader_regd_mobile_no or user_email_id'
    );
  }
  ...
  return await DeviceInfo.findOneAndUpdate(
    filter,
    update,
    options
  );
